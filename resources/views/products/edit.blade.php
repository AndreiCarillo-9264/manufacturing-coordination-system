...
<x-resource-form 
    ...
    method="PUT" 
    title="Edit Product" 
    description="Update the product details below. Fields marked with * are required." 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-5">
        <div>
            <label for="product_code" class="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">
                Product Code
            </label>
            <input type="text" 
                   id="product_code" 
                   ...
                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg font-mono font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow @error('product_code') border-red-500 ring-2 ring-red-200 @enderror">
            @error('product_code') 
        ...
        <div>
            ...
            <div class="px-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-900">
                {{ $product->date_encoded?->format('M d, Y') ?? '—' }}
            </div>
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2">
            ...
            <textarea id="description" 
                      ...
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow @error('description') border-red-500 ring-2 ring-red-200 @enderror">{{ old('description', $product->description) }}</textarea>
            @error('description')
            <p class="mt-2 text-sm text-red-600 flex items-center">
                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
            </p>
        ...
        <div class="md:col-span-2">
            ...
            <textarea id="specs" 
                      ...
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow @error('specs') border-red-500 ring-2 ring-red-200 @enderror">{{ old('specs', $product->specs) }}</textarea>
            @error('specs')
            <p class="mt-2 text-sm text-red-600 flex items-center">
                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
            </p>
        ...
        <div class="mt-6 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="text-center">
                    ...
                    <div id="diff-display" class="text-3xl font-bold text-gray-800">
                        {{ currencySymbol($product->currency ?? 'PHP') }}{{ number_format($product->diff ?? 0, 2) }}
                    </div>
                </div>
                <div class="text-center">
                    <div class="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">
                    ...
                    <div id="mu-display" class="text-3xl font-bold text-gray-800">
                        {{ number_format(($product->mu ?? 0) * 100, 2) }}%
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" 
                       id="rsqf_number" 
                       name="rsqf_number" 
                       value="{{ old('rsqf_number', $product->rsqf_number) }}"
            ...
            <div>
                ...
                <textarea id="remarks" 
                          ...
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow @error('remarks') border-red-500 ring-2 ring-red-200 @enderror">{{ old('remarks', $product->remarks) }}</textarea>
                @error('remarks')
                <p class="mt-2 text-sm text-red-600 flex items-center">
                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                </p>
...
document.addEventListener('DOMContentLoaded', () => {
    // Live Diff & MU calculation for edit form
